...
def encode_datetime(dt: datetime = None) -> bytes:
    """
    Encodes date and time into a 6-byte sequence (48 bits used):
    - Seconds (0-59): 6 bits (bits 0-5)
    - Minutes (0-59): 6 bits (bits 6-11)
    - Hours (0-23): 5 bits (bits 12-16)
    - Month (1-12): 4 bits (bits 17-20)
    - Day of Month (1-31): 5 bits (bits 21-25)
    - Day of Week (0-6): 3 bits (bits 26-28)
    - Year (0-4095): 12 bits (bits 29-40)
    - Version/Reserved: 7 bits (bits 41-47, set to 0)
    """
    if dt is None:
        dt = datetime.now()
    ...
    encoded = bytearray(6)
    # Byte 0: Seconds (6 bits, 0-5), Version (2 bits, 6-7)
    encoded[0] = (seconds & 0x3F) | ((version & 0x03) << 6)
    # Byte 1: Minutes (6 bits, 0-5), Version (2 bits, 6-7)
    encoded[1] = (minutes & 0x3F) | (((version >> 2) & 0x03) << 6)
    # Byte 2: Hours (5 bits, 0-4), Month (3 bits, 5-7)
    encoded[2] = (hours & 0x1F) | ((month & 0x07) << 5)
    # Byte 3: Month (1 bit, 0), Day of Month (5 bits, 1-5), Version (2 bits, 6-7)
    encoded[3] = ((month >> 3) & 0x01) | ((day_of_month & 0x1F) << 1) | (((version >> 4) & 0x03) << 6)
    # Byte 4: Day of Week (3 bits, 0-2), Year (5 bits, 3-7)
    encoded[4] = (day_of_week & 0x07) | ((year & 0x1F) << 3)
    # Byte 5: Year (7 bits, 0-6), Version (1 bit, 7)
    encoded[5] = ((year >> 5) & 0x7F) | (((version >> 6) & 0x01) << 7)
    return bytes(encoded)

def decode_datetime(data: bytes) -> Tuple[int, int, int, int, int, int, int, int]:
    """
    Decodes a 6-byte sequence into date/time components.
    Returns (seconds, minutes, hours, day_of_week, month, day_of_month, year, version).
    """
    if len(data) < 6:
    ...
    # Unpack 48 bits
    seconds = data[0] & 0x3F  # Bits 0-5
    minutes = data[1] & 0x3F  # Bits 6-11
    hours = data[2] & 0x1F    # Bits 12-16
    month = ((data[2] >> 5) & 0x07) | ((data[3] & 0x01) << 3)  # Bits 17-20
    day_of_month = (data[3] >> 1) & 0x1F  # Bits 21-25
    day_of_week = data[4] & 0x07         # Bits 26-28
    year = ((data[4] >> 3) & 0x1F) | ((data[5] & 0x7F) << 5)  # Bits 29-40
    version = ((data[0] >> 6) & 0x03) | (((data[1] >> 6) & 0x03) << 2) | (((data[3] >> 6) & 0x03) << 4) | (((data[5] >> 7) & 0x01) << 6)  # Bits 41-47
...
class SmartCompressor:
    ...
    def compress_with_best_method(self, data):
        dt = datetime.now()  # Current time (2025-07-16 17:57:00 IST)
    ...
    def decompress_with_best_method(self, data):
        if len(data) < 7:  # 1 byte marker + 6 bytes datetime
            logging.error("Empty or insufficient compressed data.")
            return b''
        
        method_marker = data[0]
        datetime_bytes = data[1:7]  # 6 bytes datetime

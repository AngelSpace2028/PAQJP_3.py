...
class StateTable:
    def __init__(self):
        self.table = [
            ...
            [110, 168, 0, 12], [169, 105, 20, 0], [170, 117, 19, 1], [118, 171, 1, 19],
...
def encode_datetime(dt: datetime = None) -> bytes:
    """
    Encodes date and time into a 10-byte sequence:
    - Seconds (0-59): 1 byte (base 256)
    - Minutes (0-59): 1 byte (base 256)
    - Hours (0-23): 1 byte (base 256)
    - Day of Week (0-6): 1 byte (base 256)
    - Day of Year (0-356): 2 bytes (base 512)
    - Month (1-12): 1 byte (base 256)
    - Day of Month (1-31): 1 byte (base 256)
    - Year (0-4095): 2 bytes (base 4096)
    """
    if dt is None:
        dt = datetime.now()
    
    seconds = dt.second  # 0-59
    minutes = dt.minute  # 0-59
    hours = dt.hour      # 0-23
    day_of_week = dt.weekday()  # 0-6 (Monday=0, Sunday=6)
    day_of_year = dt.timetuple().tm_yday - 1  # 0-356
    month = dt.month     # 1-12
    day_of_month = dt.day  # 1-31
    year = dt.year       # 0-4095 (assuming within range)

    # Validate ranges
    if not (0 <= seconds <= 59 and 0 <= minutes <= 59 and 0 <= hours <= 23 and
            0 <= day_of_week <= 6 and 0 <= day_of_year <= 356 and
            1 <= month <= 12 and 1 <= day_of_month <= 31 and 0 <= year <= 4095):
        logging.error("Invalid date/time values for encoding")
        raise ValueError("Date/time values out of range")

    # Encode to bytes
    encoded = bytearray()
    encoded.append(seconds)  # 1 byte
    encoded.append(minutes)  # 1 byte
    encoded.append(hours)    # 1 byte
    encoded.append(day_of_week)  # 1 byte
    encoded.append(day_of_year // 512)  # High byte for day of year
    encoded.append(day_of_year % 512)   # Low byte for day of year
    encoded.append(month)    # 1 byte
    encoded.append(day_of_month)  # 1 byte
    encoded.append(year // 256)  # High byte for year
    encoded.append(year % 256)   # Low byte for year
    
    return bytes(encoded)

def decode_datetime(data: bytes) -> Tuple[int, int, int, int, int, int, int, int]:
    """
    Decodes a 10-byte sequence into date/time components.
    Returns (seconds, minutes, hours, day_of_week, day_of_year, month, day_of_month, year).
    """
    if len(data) < 10:
    ...
    minutes = data[1]  # 0-59
    hours = data[2]    # 0-23
    day_of_week = data[3]  # 0-6
    day_of_year = (data[4] << 9) | data[5]  # Base 512: 0-356
    month = data[6]    # 1-12
    day_of_month = data[7]  # 1-31
    year = (data[8] << 8) | data[9]  # Base 4096: 0-4095
    ...
    return (seconds, minutes, hours, day_of_week, day_of_year, month, day_of_month, year)
...
def squash(d: int, n: int = 12) -> int:
    """
    Maps input d to a non-negative integer in [0, 2**n - 1] (0 to 4095 for n=12).
    Uses a logistic-like function scaled to 2**n for compression.
    """
    max_output = (1 << n) - 1  # 2**n - 1 = 4095 for n=12
    if d > 2047:
        return max_output  # Clamp to maximum
    if d < -2047:
        return 0  # Clamp to minimum
    # Logistic-like scaling: map d to [0, 2**n-1] using 2**n
    scaled = (1 << n) / (1 + math.exp(-d / 512.0))  # Smooth curve, adjusted for range
    result = int(scaled)
    return min(max(result, 0), max_output)  # Ensure non-negative and within bounds
...
class SmartCompressor:
    ...
    def transform_05(self, data, shift=3):
        """Second algorithm: Bit rotation transformation"""
        transformed = bytearray(data)
        for i in range(len(transformed)):
    ...
    def reverse_transform_05(self, data, shift=3):
        """Reverse of transform_05: Bit rotation in opposite direction"""
        transformed = bytearray(data)
        for i in range(len(transformed)):
            transformed[i] = ((transformed[i] >> shift) | (transformed[i] << (8 - shift))) & 0xFF
        return bytes(transformed)
    ...
    def transform_06(self, data, seed=42):
        """Third algorithm: Prime-based substitution"""
        random.seed(seed)
    ...
    def reverse_transform_06(self, data, seed=42):
        """Reverse of transform_06: Inverse substitution"""
        random.seed(seed)
        substitution = list(range(256))
        random.shuffle(substitution)
    ...
    def compress_with_best_method(self, data):
        ...
        dt = datetime(2025, 7, 11, 9, 26, 0)  # Use provided date/time for consistency
        ...
        if len(data) < HUFFMAN_THRESHOLD:
            ...
            compressed_bytes = int(compressed_huffman, 2).to_bytes((len(compressed_huffman) + 7) // 8, 'big')
            if len(compressed_bytes) < best_size:
                best_size = len(compressed_bytes)
                best_compressed = compressed_bytes
                best_marker = 4  # Huffman marker
                best_method = 'huffman'
        ...
        logging.info(f"Best compression method: {best_method}, Marker: {best_marker}")
        # Prepend method marker and datetime bytes
        return bytes([best_marker]) + datetime_bytes + best_compressed
    
    def decompress_with_best_method(self, data):
        if len(data) < 11:  # 1 byte marker + 10 bytes datetime
            logging.error("Empty or insufficient compressed data.")
            return b''
        
        method_marker = data[0]
        datetime_bytes = data[1:11]
        compressed_data = data[11:]
        
        # Decode datetime
        try:
            seconds, minutes, hours, day_of_week, day_of_year, month, day_of_month, year = decode_datetime(datetime_bytes)
            logging.info(f"Decoded datetime: {year}-{month:02d}-{day_of_month:02d} {hours:02d}:{minutes:02d}:{seconds:02d}, "
                         f"Day of week: {day_of_week}, Day of year: {day_of_year}")
        ...
        if method_marker == 4:  # Huffman
            ...
            if not decompressed_binary:
                logging.error("Huffman decompression failed.")
                return b''
            try:
...
def main():
    print("PAQJP_1 Compression System")
    print("Created by Jurijus Pacalovas.")
    print("Options:")
    print("1 - Compress file (PAQJP_1 with transformations and datetime)")
    print("2 - Decompress file (PAQJP_1 with transformations and datetime)")
    ...
    if choice == '1':
        ...
        compressed = compressor.compress_with_best_method(input_data)
        if compressed is None:
            return
        ...
        try:
            decompressed = compressor.decompress_with_best_method(input_data)
